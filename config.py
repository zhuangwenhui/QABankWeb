...
# 课程为固定分类。**下面这行就是唯一真源**,不要再去别处找"权威名单"。
# (曾有一份技术文档 §4 列过一份分类表,内容与此不符且已于 V1 收尾时删除。)
# 「备注」曾被当成科目使用,导致两条个人笔记(出願要点/复习计划)混进题库并显示在列表页。
# 2026-07-26 清掉那两条后从选项里移除,防止再被误选。
# 「算法」是 exam-harvest 院试题流水线入库时新增的,新增科目须与该流水线的 qb_subject 白名单一致。
SUBJECTS = ['算法', '向量解析', '复变函数', '微分方程', '微积分', '概率统计', '线性代数']
